...
var findDuplicateSubtrees = function (root) {
  ...
  let done = new Set();
  let result = new Set();
  eachQueue: for (const queue of map.values()) {
    const len = queue.length;

    eachA: for (let i = 0; i < len; i++) {
      const nodeA = queue[i];
      if (done.has(nodeA)) continue eachA;

      eachB: for (let j = i + 1; j < len; j++) {
        const nodeB = queue[j];
        if (done.has(nodeB)) continue eachB;
